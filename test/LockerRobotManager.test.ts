...
test('should_get_L_ticket_WHEN_LockerRobotManager_store_bag_GIVEN_L_bag_have_space', () => {
...
test('should_prompt_failure_WHEN_LockerRobotManager_store_bag_GIVEN_L_bag_have_no_space', () => {
  for (let i = 0; i < L_LOCKER_CAPACITY * LOCKER_ROBOT_MANAGER_DEFAULT_LOCKER_COUNT; i += 1) {
    lockerRobotManager.storeBag(lCustomerBag);
  }
  const promptMessage = lockerRobotManager.storeBag(lCustomerBag) as string;
...
test('should_L_bag_WHEN_LockerRobotManager_take_bag_GIVEN_L_ticket_and_is_invalid', () => {
  lockerRobotManager.storeBag(lCustomerBag);
  const invalidTicket = new Ticket(L_LOCKER_SIZE, 1);
  const promptMessage = lockerRobotManager.takeLBag(invalidTicket) as string;

  expect(promptMessage).toEqual(PROMPT_MESSAGE_INVALID_TICKET);
});

test('should_L_bag_WHEN_LockerRobotManager_take_bag_GIVEN_mismatch_ticket', () => {
  lockerRobotManager.storeBag(lCustomerBag);
  const mismatchTicket = new Ticket(M_LOCKER_SIZE, 1);
  const promptMessage = lockerRobotManager.takeLBag(mismatchTicket) as string;
